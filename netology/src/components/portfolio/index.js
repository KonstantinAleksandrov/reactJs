...
import '../portfolio/style.scss'
 ...
 const Portfolio = () =>{
    ...
    const changeProjects = (title) =>{
      setCategory(title)
      activeListImg = listImg.filter((item)=>item.category === title)
      ...
      return (
        <div className='portfolio'>
            <div className='nav'>
                {listTitle.map((title)=>{
                    return (
                        <div onClick={((event)=>changeProjects(event.target.textContent))} className={title === category? 'active link' : 'link'} key={title}>{title}</div>
                    )
                })}
            </div>
